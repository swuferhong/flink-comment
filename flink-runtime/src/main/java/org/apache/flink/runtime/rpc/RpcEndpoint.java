...
public abstract class RpcEndpoint implements RpcGateway, AutoCloseableAsync {
	/*TODO 抽象类，实现了一个网关接口，
	* TODO RpcEndpoint是通信终端，提供RPC组件的生命周期管理（start、stop）。每个RpcEndpoint对应一个路径（endpointId和 actorSystem共同确定）,
	* */
	...
	private final RpcService rpcService;   /*TODO 两个重要的变量，rpcService 和 rpcServer*/
	...
	public final void start() {
		/*TODO 终端的启动，实际上是由 自身网关(RpcServer)来启动的*/
		rpcServer.start();
	}
