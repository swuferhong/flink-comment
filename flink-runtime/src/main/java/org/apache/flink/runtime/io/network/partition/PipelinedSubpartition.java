...
public class PipelinedSubpartition extends ResultSubpartition
	...
	/*TODO 很重的一个数据结构，在向下游传递barrier的过程中*/
		/*TODO 包括checkpoint的时间也会放进来，老版本好像是ArrayDeque，并不支持事件优先级*/
	...
	private boolean add(BufferConsumer bufferConsumer, int partialRecordLength, boolean finish) {
		...
		synchronized (buffers) {
			...
			/*TODO 增加Buffer到PrioritizedDeque中，优先级高的buffer放到队列头*/
			if (addBuffer(bufferConsumer, partialRecordLength)) {
				prioritySequenceNumber = sequenceNumber;
			}
			updateStatistics(bufferConsumer);  /*TODO 中buffer数量+ 1*/
			increaseBuffersInBacklog(bufferConsumer);  /*TODO 中backlog数 + 1*/
		...
		if (notifyDataAvailable) {
			/*TODO 如果可用（比如数据完整，非阻塞模式等， 通知下游的inputGate来消费）*/
			notifyDataAvailable();
		}
