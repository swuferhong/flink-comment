...
public class StreamOperatorStateHandler {
	...
	void snapshotState(
		...
		try {
			...
			/*TODO 执行持久化state的操作*/
			/*TODO  比如map操作，它生成的是StreamMap属于AbstractUdfStreamOperator子类，里面封装了
			* TODO snapshotState逻辑。如果没有实现ck接口就跳过这个步骤
			* */
			...
			/*TODO DefaultOperatorStateBackend， 持久化到内存中*/
			if (null != operatorStateBackend) {
				snapshotInProgress.setOperatorStateManagedFuture(
					/*TODO 持久化当前checkpointId等属性*/
					operatorStateBackend.snapshot(checkpointId, timestamp, factory, checkpointOptions));
			}

			/*TODO 有HeapKeyedStateBackend， RocksDBKeyedStateBackend等基于Key的持久化方式*/
	...
	public interface CheckpointedStreamOperator {
		...
		/*TODO 这里要找到的是 AbstactUdfStreamOperator， （map， filter等Operator都继承了该abstract类）*/
